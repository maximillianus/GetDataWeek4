...
run_analysis <- function()
{
	...
	##save the name of required variables into features dataframe and name testtrain df
	...
	if (file.exists("tidydata_groupmean.txt") == TRUE)
	{
		file.remove("tidydata_groupmean.txt")
	}
	write.table(newdf, file="tidydata_groupmean.txt", row.names=FALSE)
	
	#Clear out variables, detach packages
	rm(list=ls())
